...
export default function SeedPage() {
  ...
  const runSeed = async () => {
    setLoading(true);
    try {
      const response = await fetch("/api/trpc/seed.runSeed", {
        method: "POST",
  ...
  const checkStatus = async () => {
    try {
      const response = await fetch("/api/trpc/seed.status");
      const data = await response.json();
      setResult({ success: true, message: `Current merchants: ${data.result?.data?.count || 0}`, count: data.result?.data?.count });
    } catch (error) {
      setResult({ success: false, message: "Cannot check status" });
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4" dir="rtl">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-4">
          ...
          >
            {loading ? (
              <><Loader2 className="h-5 w-5 animate-spin ml-2" /> جاري الاضافة...</>
            ) : (
              "اضافة 50 متجر الان"
            )}
          </Button>
