...
import {
  ...
  BadgeCheck, Globe, X, Filter, Store,
...
export default function SearchPage() {
  ...
  let merchants = merchantsData?.items || [];
  let emergencyContacts = emergencyData?.items || [];

  // Client-side filter by city
  if (selectedCity) {
    merchants = merchants.filter((m: any) => m.city === selectedCity);
    emergencyContacts = emergencyContacts.filter((e: any) => e.city === selectedCity);
  }

  // Client-side filter by category
  if (selectedCategory) {
    merchants = merchants.filter((m: any) => m.category === selectedCategory);
  }
  ...
  if (query.trim()) {
    ...
    merchants = merchants.filter((m: any) => {
      const text = `${m.businessNameAr || ''} ${m.businessName || ''} ${m.category || ''} ${m.city || ''} ${m.country || ''} ${m.tags || ''} ${m.shortDescription || ''} ${m.description || ''}`.toLowerCase();
      return text.includes(q);
    });
    emergencyContacts = emergencyContacts.filter((e: any) => {
      const text = `${e.name || ''} ${e.nameAr || ''} ${e.type || ''} ${e.city || ''} ${e.country || ''}`.toLowerCase();
      return text.includes(q);
    });
  }

  const sortedMerchants = [...merchants].sort((a: any, b: any) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => { if (e.key === "Enter") { /* search is reactive */ } };
  ...
  const totalResults = sortedMerchants.length + emergencyContacts.length;
        ...
        <div className="bg-gradient-to-br from-[#1a1a2e] to-[#16213e] py-8">
          <div className="mx-auto max-w-5xl px-4">
            ...
            <div className="flex gap-2 max-w-2xl">
              ...
              <Button onClick={() => setQuery(query)} className="bg-gradient-to-r from-amber-400 to-amber-600 hover:from-amber-500 hover:to-amber-700 text-black font-bold px-6 rounded-xl">
            ...
            <>
                ...
                <div className="mb-8">
                  <button onClick={() => setShowEmergency(!showEmergency)} className="flex items-center gap-2 mb-4 w-full">
                    ...
                    <h2 className="text-lg font-bold text-gray-900">
                      ...
                      <Badge className="mr-2 bg-red-100 text-red-700">{emergencyContacts.length}</Badge>
                    </h2>
              ...
              {totalResults === 0 && (
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-500 text-sm mb-4">جرب تغيير معايير البحث أو التصفية</p>
